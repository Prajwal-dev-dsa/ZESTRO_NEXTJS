    ...
    return (
        ...
        >
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <h3 className="text-lg md:text-xl font-extrabold text-blue-600 tracking-tight">
                                <span className="text-slate-800">Order</span> #{orderIdString?.slice(0, 8)}
                            </h3>
                            {/* Status Badge */}
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border uppercase tracking-wider ${statusColors(status)}`}>
                                {status}
                            </span>
                        </div>
            ...
            <div className="p-5 md:p-6">
                ...
                {/* --- TRACKING SECTION (Only if Assigned) --- */}
                {order.assignedDeliveryBoy && status === 'out of delivery' && (
                    <div className="mb-6">
                        <Link
                            href={`/user/track-order/${orderIdString}`}
                            className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold shadow-lg shadow-blue-200 transition-all hover:-translate-y-0.5"
                        >
