...
import { Eye, EyeOff, Leaf, Loader, Lock, Mail } from "lucide-react"
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen px-6 py-10 bg-sky-100 relative">
            ...
            <motion.div
                ...
                <p className="text-gray-700 text-lg inline-flex md:text-xl max-w-lg">Login to start shopping with us <Leaf className="size-6 text-sky-600" /></p>
            </motion.div>
            <motion.form
                ...
                <button disabled={!email || !password || loading} className={`w-full cursor-pointer font-semibold py-3 rounded-xl transition-all duration-300 shadow-md inline-flex items-center justify-center gap-2 ${email && password ? "bg-sky-600 hover:bg-sky-700 text-white" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}>
                    {loading ? <Loader className="size-6 animate-spin" /> : "Login"}
                </button>
                ...
                <button onClick={() => signIn("google", { callbackUrl: "/" })} className="w-full mt-2 flex items-center justify-center gap-3 border-gray-300 hover:border-sky-600 border-2 cursor-pointer py-3 rounded-xl text-gray-700 font-medium transition-all duration-500">
                    <Image src={googleImage} alt="Google" width={24} height={24} />
                    <span>Register with Google</span>
                </button>
            </motion.form>
            <p className="mt-4 text-gray-600">Don't have an account? <span onClick={() => router.push("/register")} className="text-sky-600 font-semibold cursor-pointer hover:underline">Register</span></p>
        </div>
    )
}
